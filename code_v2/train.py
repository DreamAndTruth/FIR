...
def train():
    #参数为数据集————此处是否需要参数？
    x = tf.placeholder(tf.float32,[None,inference.INPUT_PART_NODE],name = 'x-input')
    y_ = tf.placeholder(tf.float32,[None,inference.OUTPUT_NODE],name = 'y-input')
    #定义输入输出placeholder
    
    #——————————————————————生成测试数据————————————————————
    for i in range(inference.INPUT_PART_NUMBER):
    ...
    y = inference.inference(input_tensor,regularizer)
    ...
    cross_entroy = tf.nn.sparse_softmax_cross_entroy_with_logits(y,y_)
    ...
    learning_rate = tf.train.exponential_decay(                                              LEARNING_RATE_BASE,                                              global_step,                                              num_datastes / BATCH_SIZE,                                              LEARNING_RATE_DECAY)
    #生层指数衰减型学习率
