...
def inference(layer_name, input_tensor, now_part_node, next_part_node,
              now_part_number, next_part_number, next_time_delay, regularizer):
    '''
    构建当前层到下一层的前向传播实例
    :param layer_name: 变量空间名称
    :param input_tensor: 输入数据 size:[batch_size,input_size]
    :param now_part_node: 当前层模块节点数目
    :param next_part_node: 下一层模块节点数目
    :param now_part_number: 当前层模块数目
    :param next_part_number: 下一层模块数目
    :param next_time_delay: 下一层时间延迟数目
    :param regularizer: 正则化项
    :return: next_layer_input:[batch_size,*]
    '''
    with tf.variable_scope(layer_name):
        weights = get_weights_variable('weights',
                                       [now_part_node*next_time_delay,
                                        next_part_node], regularizer)
        biases = get_biases_variable('biases', [1, next_part_node*next_part_number])
        ...
        for i in range(now_part_number - next_time_delay + 1):
            ...
            # now_part_number实际为输入时间序列长度
                ...
                temp = tf.matmul(input_tensor[:, i*now_part_node:(i+next_time_delay)*now_part_node], weights)
                ...
                if i == 0:
                    if (now_part_number - next_time_delay + 1) == 1:
                        ...
                        next_layer_input = tf.Variable(np.zeros([constant.BATCH_SIZE, next_part_node],
